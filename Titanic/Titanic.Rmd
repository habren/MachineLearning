...
gbmmodel <- gbm(
                ...
                verbose = FALSE)
gbm.perf(gbmmodel)
gbm.predict <- predict.gbm(gbmmodel, newdata=cv.test, n.trees = 2564)
...
assemble.predict <- function(data) {
  ...
  gbm.predict <- factor(ifelse(predict(gbmmodel, newdata=data, type="response", n.trees = 2564) > 0.5, 1, 0), levels=c(0, 1))



  assemble.result <- as.integer(lr.predict) + as.integer(rf.predict) + as.integer(svm.predict) + as.integer(cit.predict) + as.integer(gbm.predict)
  assemble.result <- ifelse(assemble.result >= 8 , 1, 0)

  return(data.frame(LinearRegression=lr.predict, RandomForest=rf.predict, SVM=svm.predict, CIT=cit.predict, GBM=gbm.predict, Ensemble=assemble.result))
}
```
