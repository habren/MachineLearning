...
test.result <- cbind(full$PassengerId[(nrow(train)+1):(nrow(full))], test.predict)
```

```{r 'Predicted result saving'}
for (name in colnames(test.result[, -1])) {
  output.data <- data.frame(PassengerId = test.result[, 1], Survived = test.result[[name]])
  write.csv(output.data, file=paste('result/', name, '.result.csv', sep = ''), row.names = FALSE, quote = FALSE)
}
```
