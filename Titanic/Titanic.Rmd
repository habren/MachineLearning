...
print(rfmodel)
print(svmmodel)
cv.lr.predict <- factor(ifelse(predict(lrmodel, newdata=cv.test, type="response") > 0.5, 1, 0), levels=c(0, 1))
cv.rf.predict <- predict(rfmodel, newdata=cv.test, type="response")
cv.svm.predict <- predict(svmmodel, newdata=cv.test, type="response")
cv.combined.predict <- as.integer(cv.lr.predict) + as.integer(cv.rf.predict) + as.integer(cv.svm.predict)
cv.combined.predict <- ifelse(cv.combined.predict >= 5 , 1, 0)

cv.predict <- data.frame(LinearRegression=cv.lr.predict, RandomForest=cv.rf.predict, SVM=cv.svm.predict, Ensemble=cv.combined.predict)
head(cv.predict, 200)
sapply(colnames(cv.predict), function(name){
  str(cv.predict[[name]])
  mode(cv.predict[[name]])
  print(name)
  print(paste(' Recall:', Recall(cv.test_true, cv.predict[[name]])))
  print(paste(' Precision:', Precision(cv.test_true, cv.predict[[name]])))
  print(paste(' Accuracy', Accuracy(cv.predict[[name]], cv.test_true)))
  print(paste(' AUC', AUC(cv.predict[[name]], cv.test_true)))
  print('')
})
...
lr.predict <- factor(ifelse(predict(lrmodel, newdata=test.data, type="response") > 0.5, 1, 0), levels=c(0, 1))


combined.predict <- as.integer(svm.predict) + as.integer(rf.predict) + as.integer(lr.predict)
combined.predict <- sapply(combined.predict, function(x){if(x >= 6){1} else{0}})
