...
cv.predict <- data.frame(LinearRegression=cv_lr_predict_test, RandomForest=cv_rf_predict_test, SVM=cv_svm_predict_test, Ensemble=cv_combined_predict_test)
sapply(colnames(cv.predict), function(name){
  print(name)
  print(paste(' Recall:', Recall(cv_test_true, cv.predict[[name]])))
  print(paste(' Precision:', Precision(cv_test_true, cv.predict[[name]])))
  print(paste(' Accuracy', Accuracy(cv.predict[[name]], cv_test_true)))
  print(paste(' AUC', AUC(cv.predict[[name]], cv_test_true)))
  print('')
})
